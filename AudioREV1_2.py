...
def detect_mid_word_clips(audio_file, energy_threshold=0.05, zcr_threshold=0.1, edge_frames=1500):
    """
    Detect if audio starts/ends mid-word using energy and zero-crossing rate.
    
    Parameters:
    - audio_file: Path to audio file
    - energy_threshold: RMS energy threshold for active speech
    - zcr_threshold: Zero-crossing rate threshold (higher for consonants)
    - edge_frames: Number of frames to analyze at edges
    """
    try:
        # Load audio
        y, sr = librosa.load(audio_file, sr=None)
        
        # Get RMS energy and zero-crossing rate
        rms = librosa.feature.rms(y=y)[0]
        zcr = librosa.feature.zero_crossing_rate(y)[0]
        
        # Analyze start of audio
        start_energy = np.mean(rms[:min(edge_frames, len(rms))])
        start_zcr = np.mean(zcr[:min(edge_frames, len(zcr))])
        
        # Analyze end of audio
        end_energy = np.mean(rms[-min(edge_frames, len(rms)):])
        end_zcr = np.mean(zcr[-min(edge_frames, len(zcr)):])
        
        # Compare to middle section for reference
        mid_start = len(rms) // 3
        mid_end = 2 * len(rms) // 3
        mid_energy = np.mean(rms[mid_start:mid_end])
        mid_zcr = np.mean(zcr[mid_start:mid_end])
        
        # Check if starts/ends mid-word
        starts_mid_word = (start_energy > energy_threshold * mid_energy) and (start_zcr > zcr_threshold * mid_zcr)
        ends_mid_word = (end_energy > energy_threshold * mid_energy) and (end_zcr > zcr_threshold * mid_zcr)
        audio_length_seconds = librosa.get_duration(y=y, sr=sr)
        
        return {
            "starts_mid_word": starts_mid_word,
            "ends_mid_word": ends_mid_word,
            "confidence": {
                "start": start_energy / mid_energy,
                "end": end_energy / mid_energy
            },
...
def calculate_audio_length(audio_file):
    try:
        ...
        print(f"Error calculating audio length for {audio_file}: {e}")
        return 0

def process_audio_folder(folder_path, output_csv=None):
    """Process all audio files in a folder and return/save results"""
    folder = Path(folder_path)
    audio_extensions = [".mp3", ".wav", ".flac", ".ogg", ".m4a"]
    audio_files = [f for ext in audio_extensions for f in folder.glob(f"*{ext}")]
...
class AudioReviewApp(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(filter_frame, text="CU <=").grid(row=1, column=8, padx=(5, 15), pady=2, sticky="w")
        ...
        ttk.Label(filter_frame, text="Length >=").grid(row=2, column=6, padx=(15, 5), pady=2, sticky="w")
        self.filter_length_min = ttk.Entry(filter_frame, width=8); self.filter_length_min.grid(row=2, column=7, padx=5, pady=2, sticky="ew")
        ttk.Label(filter_frame, text="Length <=").grid(row=2, column=8, padx=5, pady=2, sticky="w")
        self.filter_length_max = ttk.Entry(filter_frame, width=8); self.filter_length_max.grid(row=2, column=9, padx=5, pady=2, sticky="ew")
        ...
        self.tree = ttk.Treeview(tree_frame, columns=('Filename', 'CE', 'CU', 'PC', 'PQ', 'Start Mid-Word', 'End Mid-Word', 'Length', 'Path'), show='headings', selectmode='extended')
        ...
        self.tree.heading('Start Mid-Word', text='Start Mid-Word', anchor=tk.CENTER, command=lambda: self.sort_column('starts_mid_word'))
        self.tree.column('Start Mid-Word', anchor=tk.CENTER, width=100, stretch=tk.NO)
        self.tree.heading('End Mid-Word', text='End Mid-Word', anchor=tk.CENTER, command=lambda: self.sort_column('ends_mid_word'))
        self.tree.column('End Mid-Word', anchor=tk.CENTER, width=100, stretch=tk.NO)
        self.tree.heading('Length', text='Length (s)', anchor=tk.E, command=lambda: self.sort_column('audio_length_seconds', True))
        self.tree.column('Length', anchor=tk.E, width=100, stretch=tk.NO)
    ...
    def apply_filters(self):
        ...
        try:
            for entry in self.full_audio_data:
                ...
                if length_min is not None and (entry['audio_length_seconds'] is None or entry['audio_length_seconds'] < length_min): continue
                if length_max is not None and (entry['audio_length_seconds'] is None or entry['audio_length_seconds'] > length_max): continue
                if starts_mid_word_filter and not entry['starts_mid_word']: continue
    ...
    def clear_filters(self):
        ...
        self.filter_length_min.delete(0, tk.END); self.filter_length_max.delete(0, tk.END)
        self.filter_mid_word_start_var.set(False)
        self.filter_mid_word_end_var.set(False)
        self.apply_filters()
